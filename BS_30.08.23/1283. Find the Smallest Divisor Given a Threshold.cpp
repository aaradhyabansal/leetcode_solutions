...
public:
    ...
    bool DivisorPossible(vector<int>& nums, int threshold,int check)
    {
        int sumC=0;
        for(int i=0;i<nums.size();i++)
        {
            sumC+=ceil(1.0*nums[i]/check);
            if(sumC>threshold)
            break;
        }
        return sumC<=threshold?1:0;
    }
    ...
    int smallestDivisor(vector<int>& nums, int threshold) {
        ...
        while(low<=high)
        {
            ...
            if(DivisorPossible(nums,threshold,mid)==1)
            {
                high=mid-1;
            }
            else
